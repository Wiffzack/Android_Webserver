...
multip = 2
#msgl = numpy.array([''])
Mylist = [''];blockl = [''];index = [];fpart = [''];blocklw = [''];response = '';chatn = ''
threads = []
workn = 1
data = ''
...
def handleMessage(msg):
	...
	def createmp():
		try:
			try:
				...
				return
			try:
				threading2 = threading.Thread(target=chbot)
	...
	def chbot():
		...
		#	input1 = simplejson.load(f)

		# Train the chat bot with a few responses
		input1 = ([
			'Welche sind Grundeinheiten des internationalen Einheitensystems?',
			'Kilogramm(kg ) Sekunde (s) Meter(m) Candela (cd) Kelvin (K) Ampere (A) Mol (mol)',
			'Wie ist das Kilo definiert ?',
			'Kilo : Seine Masse ist festgelegt durch den Internationalen Kilogrammprototyp (auch Urkilogramm), einen Zylinder aus Platin-Iridium.',
			'Wie ist die Sekunde definiert ?',
			'Eine Sekunde ist das 9.192.631.770 Fache der Periodendauer der Strahlung, die dem Uebergang zwischen den beiden Hyperfeinstrukturniveaus des Grundzustandes von Atomen des Nuklids 133Cs entspricht.',
			'Wie ist ein Meter definiert?',
			'Ein Meter ist definiert als die Laenge der Strecke, die das Licht im Vakuum waehrend der Dauer von 1/299 792 458 Sekunde zuruecklegt.',
			'Wie lautet das erste Newtische Axiom ?',
			'Ein kraeftefreier Koerper bleibt in Ruhe oder bewegt sich geradlinig mit konstanter Geschwindigkeit.',
			'Wie lautet das zweite Newtische Axiom ?',
			'Kraft gleich Masse mal Beschleunigung.',
			'Wie lautet das dritte Newtische Axiom ?',
			'Kraft gleich Gegenkraft  Eine Kraft von Koerper A auf Koerper B geht immer mit einer gleich grossen aber entgegen gerichteten Kraft von Koerper B auf Koerper A einher.',
			'Wie lautet der sinn der zwiebel ?',
			' $$x = {-b \pm \sqrt{b^2-4ac} \over 2a}.$$'
			])
		bot.train(input1)
		try:
			if os.path.isfile("../database.db"):
				if not (os.stat("../database.db").st_size == 0):
					pass
				else:
				...
				raise ValueError('database file doesnt exist!!!')
		except:
			pass
		while True:
			time.sleep(1)
			response = bot.get_response(chatn)
	...
	# Assumtion 4 Threads : everybody knows this can not work all the time !
	def stranasplit(msg):
...
# Speed increase with jython isnt possible at the moment
	@copy_current_request_context
	def textanafunc(msg):
		from multiprocessing import Process, Manager, freeze_support
		import pymp
		global fpart,test,chatn
		...
		send(cmplzstring(str(listo)), room=1)
		send(cmplzstring(str(response)), room=1)
		test = '';fpart = ['']
		...
		try:
			...
			if any((c in chars) for c in msg):
				...
				try:
					...
					pass
					send(cmplzstring(str(msg)), room=1)
...
def cmpstrzlib(data):
	...
	compd = zlib_compress.compress(data) + zlib_compress.flush()
	compd = base64.b64encode(compd)
	return (compd)
...
def deflate(data, compresslevel=-1):
